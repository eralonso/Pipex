...
then
	...
	echo "\033[1;94mTEST 10:\nLEAK_RETURN == $LEAK_RETURN\033[0;m"
	printf "${result_color}# $num: %-69s [%s]\n${NC}" "$description" "$result"
else
	printf "\n"
fi
...
then
	...
	then
		...
		then
			TESTS_LK=$(($TESTS_LK + 1))
			result="LK"
		else./pipex "/dev/urandom" "cat" "head -1" "outs/test-xx.txt"
			TESTS_KO=$(($TESTS_KO + 1))
			result="KO"
		fi
		result_color=$RED
	fi
	echo "\033[1;94mTEST 17:\nLEAK_RETURN == $LEAK_RETURN\033[0;m"
	printf "${result_color}# $num: %-69s [%s]\n${NC}" "$description" "$result"
else
	printf "\n"
fi
...
then
	...
	then
		...
		then
			echo "\033[1;94mLEAK_RETURNN == $LEAK_RETURN"
			TESTS_LK=$(($TESTS_LK + 1))
			result="LK"
		else
			TESTS_KO=$(($TESTS_KO + 1))
